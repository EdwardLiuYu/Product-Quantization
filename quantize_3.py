...
parser.add_argument('--n-centroids-fc', default=512, type=int,
...
parser.add_argument('--data-path', default='/mnt/xfs1/home/IVA/data/ilsvrc12_torch', type=str,
...
def main():
    ...
    # student = nn.DataParallel(student,[4,5,6])
    student.eval()
    criterion = nn.CrossEntropyLoss().cuda()
    cudnn.benchmark = True
    
    # layers to quantize (we do not quantize the first 7x7 convolution layer)
    watcher = ActivationWatcher(student,is_hook=False)
    # print(watcher.layers[1:])
    ...
    # teacher = nn.DataParallel(teacher,[4,5,6])
    teacher.eval()
    ...
    for layer in layers:
        # dynamic choose n_activations
        args.n_activations = dynamic_n_activations(layer)
        #  gather input activations
        n_iter_activations = math.ceil(args.n_activations / args.batch_size)
        watcher = ActivationWatcher(student, layer=layer,is_hook=True)
        ...
        print('in_activations_current_size',in_activations_current.size())
        ...
        if is_conv:
            ...
            n_centroids = dynamic_centroids(layer)
        ...
        if len(args.restart) > 0:
            ...
            try:
                ...
                parameters = [p for (n, p) in student.named_parameters() if layer + '.weight' == n]
